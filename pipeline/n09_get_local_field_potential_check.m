%%% Micros Pipeline, Step 9 Check
%%% Following the resampling of a microelectrode recording's data
...
function n09_get_local_field_potential_check(varargin)
...
check_directory = fullfile(root_directory, 'micros_pipeline/process_files/n09_lfp/check');
process_directory = fullfile(root_directory, 'micros_pipeline/process_files/n09_lfp/lock_files');
...
if is_leader_node
    ...
    fprintf('%s - Node %d performing step 9 check.\n', datestr(now), feature('getpid'));
    ...
    try
        ...
        for ldx = 1:length(lock_files)
            ...
            elseif had_error
                ...
                this_error_directory = fullfile(error_directory, subject, folder, 'lfp', sprintf('Bank%s', bank));
        ...
        pause(10)
        fprintf('%s - Node %d errored in step 9 check and quit.\n', datestr(now), feature('getpid'));
    ...
    if isfile(stop_file)
        fprintf('%s - Node %d quit while waiting for step 9 check completion.\n', datestr(now), feature('getpid'));
        quit
    end
    fprintf('%s - Node %d finished waiting for step 9 check completion.\n', datestr(now), feature('getpid'));
end
end