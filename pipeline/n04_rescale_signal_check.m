%%% Micros Pipeline, Step 4 Check
%%% Following the rescaling of a microelectrode recording's data
...
function n04_rescale_signal_check(varargin)
...
check_directory = fullfile(root_directory, 'micros_pipeline/process_files/n04_rescale/check');
process_directory = fullfile(root_directory, 'micros_pipeline/process_files/n04_rescale/lock_files');
...
if is_leader_node
    ...
    fprintf('%s - Node %d performing step 4 check.\n', datestr(now), feature('getpid'));
    ...
    try
        ...
        for ldx = 1:length(lock_files)
            ...
            elseif had_error
                ...
                this_error_directory = fullfile(error_directory, subject, folder, 'rescaled', sprintf('Bank%s', bank));
        ...
        pause(10)
        fprintf('%s - Node %d completed step 4 check.\n', datestr(now), feature('getpid'));
        ...
        pause(10)
        fprintf('%s - Node %d errored in step 4 check and quit.\n', datestr(now), feature('getpid'));
        file_id = fopen(stop_file, 'w');
        fprintf(file_id, '%s\n', strcat(datestr(now), ' - Step 4 Check Error:'));
    ...
    if isfile(stop_file)
        fprintf('%s - Node %d quit while waiting for step 4 check completion.\n', datestr(now), feature('getpid'));
        quit
    end
    fprintf('%s - Node %d finished waiting for step 4 check completion.\n', datestr(now), feature('getpid'));
end
end