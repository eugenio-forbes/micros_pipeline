%%% Micros Pipeline, Step 5 Check
%%% Following the .hdf5 conversion of a microelectrode recording's data
...
function n05_convert_to_hdf5_check(varargin)
...
check_directory = fullfile(root_directory, 'micros_pipeline/process_files/n05_hdf5/check');
process_directory = fullfile(root_directory, 'micros_pipeline/process_files/n05_hdf5/lock_files');
...
if is_leader_node
    ...
    fprintf('%s - Node %d performing step 5 check.\n', datestr(now), feature('getpid'));
    ...
    try
        ...
        for ldx = 1:length(lock_files)
            ...
            elseif had_error
                ...
                this_error_directory = fullfile(error_directory, subject, folder, 'hdf5', sprintf('Bank%s', bank));
        ...
        pause(10)
        fprintf('%s - Node %d completed step 5 check.\n', datestr(now), feature('getpid'));
        ...
        pause(10)
        fprintf('%s - Node %d errored in step 5 check and quit.\n', datestr(now), feature('getpid'));
        file_id = fopen(stop_file, 'w');
        fprintf(file_id, '%s\n', strcat(datestr(now), ' - Step 5 Check Error:'));
    ...
    if isfile(stop_file)
        fprintf('%s - Node %d quit while waiting for step 5 check completion.\n', datestr(now), feature('getpid'));
        quit
    end
    fprintf('%s - Node %d finished waiting for step 5 check completion.\n', datestr(now), feature('getpid'));
end
end