%%% Micros Pipeline, Step 6 Check
%%% Following the spike clustering of a microelectrode recording's data
...
%%% Additionally will add to table date that clustering occured,
%%% and create a lock file for step 7, in order to keep track of which
%%% clustering results have been manually reviewed.


function n06_cluster_signal_spikes_check(varargin)
...
check_directory = fullfile(root_directory, 'micros_pipeline/process_files/n06_cluster/check');
process_directory = fullfile(root_directory, 'micros_pipeline/process_files/n06_cluster/lock_files');
manual_directory = fullfile(root_directory, 'micros_pipeline/process_files/n07_manual/lock_files');
...
if is_leader_node
    ...
    fprintf('%s - Node %d performing step 6 check.\n', datestr(now), feature('getpid'));
    ...
    try
        ...
        for ldx = 1:length(lock_files)
            ...
            if is_done
                ...
                cluster_date = get_cluster_date(data_directory, subject, folder, bank);
                
                if ~isempty(cluster_date)
                    updated_row.clustered = true;
                    updated_row.cluster_date = cluster_date;
                    
                    create_manual_lock(manual_directory, subject, folder, bank); %%% Create a *_lock for manual results review. To be renamed to *_done manually.
            ...
            elseif had_error
                ...
                this_error_directory = fullfile(error_directory, subject, folder, 'cluster', sprintf('Bank%s', bank));
        ...
        pause(10)
        fprintf('%s - Node %d completed step 6 check.\n', datestr(now), feature('getpid'));
        ...
        pause(10)
        fprintf('%s - Node %d errored in step 6 check and quit.\n', datestr(now), feature('getpid'));
        file_id = fopen(stop_file, 'w');
        fprintf(file_id, '%s\n', strcat(datestr(now), ' - Step 6 Check Error:'));
    ...
    if isfile(stop_file)
        fprintf('%s - Node %d quit while waiting for step 6 check completion.\n', datestr(now), feature('getpid'));
        quit
    end
    fprintf('%s - Node %d finished waiting for step 6 check completion.\n', datestr(now), feature('getpid'));
end
end


function create_manual_lock(manual_directory, subject, folder, bank)
manual_lock = fullfile(manual_directory, strcat(subject, '_', folder, '_', bank, '_lock'));
if ~isfile(manual_lock)
    file_id = fopen(manual_lock, 'w'); fclose(file_id);
end
end


function cluster_date = get_cluster_date(data_directory, subject, folder, bank)
...
cluster_date = max([sort_files.datenum]);
end
