%%% Micros Pipeline, Step 8 Check
%%% Following the extraction of spike data and quality measures
%%% from Combinato's clustering results, 
...
%%% Additionally will add to table whether a recording yielded single unit activity,
%%% the channels that yielded single unit activity, and the interspike interval noise ratio.

function n08_gather_spike_data_check(varargin)
...
%%% Declare directories and add path to pipeline code
data_directory = fullfile(root_directory, 'micros_database');
check_directory = fullfile(root_directory, 'micros_pipeline/process_files/n08_spike_data/check');
process_directory = fullfile(root_directory, 'micros_pipeline/process_files/n08_spike_data/lock_files');
...
if is_leader_node
    ...
    fprintf('%s - Node %d performing step 8 check.\n', datestr(now), feature('getpid'));
    ...
    try
        ...
        for ldx = 1:length(lock_files)
            ...
            if is_done
                ...
                %%% Get spike data at channel level
                load(fullfile(spike_directory, 'channels.mat'), 'channels');
                if isempty(channels)
                    channels.isi_SNR = [];
                    channels.n_SUA = 0;
                    channels.channel_number = 0;
                end
                
                %%% Update whether SUA was found in any channel recording, channel numbers where found, and interspike interval noise ratio
                updated_row.spikes_timed = true;
                updated_row.modified = false; %%% Updated data, set modified back to false.
                updated_row.single_units = any([channels.n_SUA] > 0);
                updated_row.single_unit_channels = {[channels([channels.n_SUA] > 0).channel_number]'};
                updated_row.isi_SNR = {[channels.isi_SNR]'};
            ...
            elseif had_error
                ...
                this_error_directory = fullfile(error_directory, subject, folder, 'spike_data', sprintf('Bank%s', bank));
        ...
        pause(10)
        fprintf('%s - Node %d completed step 8 check.\n', datestr(now), feature('getpid'));
        ...
        pause(10)
        fprintf('%s - Node %d errored in step 8 check and quit.\n', datestr(now), feature('getpid'));
        file_id = fopen(stop_file, 'w');
        fprintf(file_id, '%s\n', strcat(datestr(now), ' - Step 8 Check Error:'));
    ...
    if isfile(stop_file)
        fprintf('%s - Node %d quit while waiting for step 8 check completion.\n', datestr(now), feature('getpid'));
        quit
    end
    fprintf('%s - Node %d finished waiting for step 8 check completion.\n', datestr(now), feature('getpid'));
end
end